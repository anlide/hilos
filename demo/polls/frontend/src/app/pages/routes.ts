// The poll page routes: the cold-load route registry the navigator resolves a
// URL against. The route engine and the framework admin catalog come from
// @hilos/core, so this file only declares the poll's own pages;
...
/**
 * The poll's own pages keyed to the URL path template each answers and whether
 * that page is an administrative surface. The framework admin pages are merged
 * in by `createAppPageRouter` rather than restated here.
 */
