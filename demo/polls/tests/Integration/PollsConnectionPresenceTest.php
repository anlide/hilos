 ...
 */
final class PollsConnectionPresenceTest extends IntegrationTestCase
{
    ...
    {
        Hilos::$rt->connections->actions->register('polls-ak-1', 7, self::SESSION_TOKEN_A);

        $connection = Hilos::$rt->connections['polls-ak-1'];
     ...
     *
     * The ordinary state of a visitor since HIL-611: the row exists, it names the
    ...
    {
        Hilos::$rt->connections->actions->register('polls-ak-anon', null, self::SESSION_TOKEN_A);

        $connection = Hilos::$rt->connections['polls-ak-anon'];
        $this->assertNotNull($connection);
    ...
    {
        Hilos::$rt->connections->actions->register('polls-ak-1', 7, self::SESSION_TOKEN_A);
        Hilos::$rt->connections->actions->register('polls-ak-2', 7, self::SESSION_TOKEN_B);
    ...
    {
        Hilos::$rt->connections->actions->register('polls-ak-1', 7, self::SESSION_TOKEN_A);
        Hilos::$rt->connections['polls-ak-1']?->actions->unregister();

        $this->assertNull(Hilos::$rt->connections['polls-ak-1']);
    ...
    public function testAgentConnectionCloseUnregisters(): void
    {
        ...
        new PollsAgent()->onSignalConnectionClose(new WebSocketCloseSignalDTO('polls-ak-1'), '', '');
     ...
     *
     * A new cookie resolves to a fresh anonymous session, and since HIL-611 that is
    ...
    public function testHandshakeRegistersConnection(): void
    {
        ...
        try {
            ...
            $this->deliverHandshake(new PollsAgent(), new WebSocketHandshakeSignalDTO(
                headers: [],
                acceptKey: 'polls-ak-handshake',
                cookies: [],
                clientIp: '127.0.0.1',
            ...
            $connection = Hilos::$rt->connections['polls-ak-handshake'];
            $this->assertNotNull($connection);
