 ...
 */
final class PollsAgentGuestHandshakeTest extends IntegrationTestCase
{
    ...
    public function testAnonymousHandshakeNamesAGuestAndMintsNoUser(): void
    {
        ...
        $this->handshake($token, 'polls-guest-ak');
        ...
        $connection = Hilos::$rt->connections['polls-guest-ak'];
    ...
    public function testReturningVisitorKeepsTheSameGuestName(): void
    {
        ...
        $this->handshake($token, 'polls-guest-ak-1');
        $first = $this->guests()->findBySessionToken($token);
        self::assertNotNull($first);

        $this->handshake($token, 'polls-guest-ak-2');
    ...
    public function testHandshakeOfAnAccountClearsItsGuestRow(): void
    {
        ...
        $this->handshake($token, 'polls-guest-ak-3');
        ...
        $this->handshake($token, 'polls-guest-ak-4');

        self::assertNull($this->guests()->findBySessionToken($token));

        $connection = Hilos::$rt->connections['polls-guest-ak-4'];
        self::assertNotNull($connection);
    ...
    {
        $collection = Hilos::$db->getObjectCollection(PollsDbContext::guests);
