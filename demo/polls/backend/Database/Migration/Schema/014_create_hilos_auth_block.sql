-- Migration: Create hilos_auth_block table
-- Created: 2026-08-28
-- Index: 014
-- Description:
--   The durable half of the anti-abuse ladder (HIL-420), which this demo turns
--   on together with sign-in as the AUTH_THROTTLE feature. Hot per-window
--   counters stay in runtime and never reach the DB; only a block that actually
--   fired is written here, so it survives a restart and every worker sees it.
--
--   Copied from the framework stub Database/Migration/Stub/create_hilos_auth_block.sql.
--
