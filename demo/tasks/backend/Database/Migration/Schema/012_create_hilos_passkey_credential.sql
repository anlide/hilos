...
-- Index: 012
-- Description:
--   The WebAuthn credential sidecar (HIL-284). A passkey is an identity anchor
--   row in 010 plus one crypto row here; the framework library reads both when
--   a device key signs in. The demo does not offer the device key on an empty
--   field (HIL-418), so the table stays empty until an account enrolls one.
--
--   Copied from the framework stub
--   Database/Migration/Stub/create_hilos_passkey_credential.sql.
--
