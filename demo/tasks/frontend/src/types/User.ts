// The tasks user entity and its collection. The framework owns the user's
// authorization contract (admin/block), so the domain user extends the SDK
// `User` and adds only the tasks demo's own profile fields. The `users` slot of the
...
/** The tasks user: the framework authorization fields plus tasks profile fields. */
