...
// Dev/build config for the tasks demo — the end project that consumes
...
export default defineConfig(({ mode }) => {
  // VITE_DISABLE_HMR is set by docker-compose.preview.yml to turn HMR off under
  // the TLS preview proxy; normal localhost dev leaves it unset.
  ...
  return {
    ...
    server: {
      ...
      port: 5173,
      strictPort: true,
      // Allow the in-tailnet preview names (tasks.hilos, ws.tasks.hilos, ...) so the
