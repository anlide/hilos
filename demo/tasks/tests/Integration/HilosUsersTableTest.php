...
/**
 * Integration tests for the tasks Hilos users table — the merge of DB
...
final class HilosUsersTableTest extends IntegrationTestCase
{
    ...
    public function testRowFromUserReflectsOnlinePresence(): void
    {
        $user = Hilos::$db->users->actions->registerAdmin();
        $userId = (int) $user->id;
        Hilos::$rt->connections->actions->register('tasks-ak-a', $userId);
        Hilos::$rt->connections->actions->register('tasks-ak-b', $userId);
