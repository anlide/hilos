...
/**
 * Proves the handshake leaves a visitor outside the user table (HIL-610).
 *
 ...
 */
final class TasksAgentGuestHandshakeTest extends IntegrationTestCase
{
    ...
    public function testAnonymousHandshakeNamesAGuestAndMintsNoUser(): void
    {
        ...
        $this->handshake($token, 'tasks-guest-ak');
        ...
        $connection = Hilos::$rt->connections['tasks-guest-ak'];
    ...
    public function testReturningVisitorKeepsTheSameGuestName(): void
    {
        ...
        $this->handshake($token, 'tasks-guest-ak-1');
        $first = $this->guests()->findBySessionToken($token);
        self::assertNotNull($first);

        $this->handshake($token, 'tasks-guest-ak-2');
    ...
    public function testHandshakeOfAnAccountClearsItsGuestRow(): void
    {
        ...
        $this->handshake($token, 'tasks-guest-ak-3');
        ...
        $this->handshake($token, 'tasks-guest-ak-4');

        self::assertNull($this->guests()->findBySessionToken($token));

        $connection = Hilos::$rt->connections['tasks-guest-ak-4'];
        self::assertNotNull($connection);
    ...
    {
        $collection = Hilos::$db->getObjectCollection(TasksDbContext::guests);
