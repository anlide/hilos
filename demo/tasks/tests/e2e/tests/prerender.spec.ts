...
// HTML through React's static server renderer, so a crawler — or any no-JS
// fetch — sees the page content without running the SPA. nginx serves
// <route>.html for the public routes and falls back to the SPA shell for the
// app's own deep links. These checks fetch the raw HTML (request, not a
// browser), so they assert the prerendered markup itself.
...
test('a public page is prerendered with its content and title', async ({
  ...
  expect(html).toContain('Hilos Tasks is a demonstration')
