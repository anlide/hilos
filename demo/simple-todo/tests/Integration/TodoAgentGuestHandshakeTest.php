 ...
 */
final class TodoAgentGuestHandshakeTest extends IntegrationTestCase
{
    ...
    public function testAnonymousHandshakeNamesAGuestAndMintsNoUser(): void
    {
        ...
        $this->handshake($token, 'todo-guest-ak');
        ...
        $connection = Hilos::$rt->connections['todo-guest-ak'];
    ...
    public function testReturningVisitorKeepsTheSameGuestName(): void
    {
        ...
        $this->handshake($token, 'todo-guest-ak-1');
        $first = $this->guests()->findBySessionToken($token);
        self::assertNotNull($first);

        $this->handshake($token, 'todo-guest-ak-2');
    ...
    public function testHandshakeOfAnAccountClearsItsGuestRow(): void
    {
        ...
        $this->handshake($token, 'todo-guest-ak-3');
        ...
        $this->handshake($token, 'todo-guest-ak-4');

        self::assertNull($this->guests()->findBySessionToken($token));

        $connection = Hilos::$rt->connections['todo-guest-ak-4'];
        self::assertNotNull($connection);
    ...
    {
        $collection = Hilos::$db->getObjectCollection(TodoDbContext::guests);
