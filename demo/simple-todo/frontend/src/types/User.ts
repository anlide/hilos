// The todo user entity and its collection. The framework owns the user's
// authorization contract (admin/block), so the domain user extends the SDK
// `User` and adds only the todo's own profile fields. The `users` slot of the
...
/** The todo user: the framework authorization fields plus todo profile fields. */
