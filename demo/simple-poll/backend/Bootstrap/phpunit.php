...
/**
 * PHPUnit bootstrap for the demo simple-poll package tests.
 *
 * Loads the Composer autoloader from demo/simple-poll, then populates the
 * Hilos env accessor (Hilos::$env) from the project .env and overrides it with
 * demo/simple-poll/tests/.env when present. Values are read through
 * Hilos::$env, not the process environment.
 *
 * Bootstrap failures are logged and terminate with ExitCode::ERROR, matching
 * the daemon/cli/worker entry points.
 */
...
try {
    ...
    Logger::error('Demo simple-poll PHPUnit bootstrap failed: ' . $e->getMessage(), [
