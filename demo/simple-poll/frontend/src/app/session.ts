// The poll session identity (session bootstrap): a client-minted persistent
// token rides a cookie into the WebSocket handshake; the backend resolves an
// agent-local user by it and answers with handshake_response carrying the
// session scope payload. The normalizer ingests it here and the current user
// resolves reactively. The cookie name is project-owned — the backend
// counterpart is the CookieNames constant.
import {
  computedSignal,
  ingest,
  ScopeManager,
...
/** Concrete poll signal schemas for the core parse boundary. */
export const pollSignalSchemas = {
...
export const currentUserName: ReadonlySignal<string> = computedSignal(() => {
  ...
  // Bracket access: the Angular CLI tsconfig enables
  // noPropertyAccessFromIndexSignature, and entity fields are an index signature.
