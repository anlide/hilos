 ...
 *
 * What is left of it after HIL-622 is provider wiring and a name: resolving the account a
 * finished exchange belongs to - and refusing to seize one on a shared address - is the
 * users library's command now, reached by the hand-off frame the framework agent sends.
 *
 * {@see buildProviderRegistry()} names the providers from {@see ChatOAuthConfig}.
 * {@see displayNameFor()} stays overridden for one reason only: a chat account's name has
 * to fit the frame the profile rename applies, and that length is this project's table's,
 * not the framework's.
 */
final class OAuthAgent extends AbstractOAuthAgent
{
    /**
     * Builds the chat provider registry: a real provider for each configured client pair, none otherwise.
     *
