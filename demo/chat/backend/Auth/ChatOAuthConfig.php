 ...
 * {@see OAuthAgent} drives. When a provider's client id + secret are configured a
 * real {@see GenericOAuthProvider} is built over the framework's preset; when either
 * is empty the provider is left out - no icon, no sign-in - on every node alike
 * (HIL-924). The test stand fills a fake pair and points the presets at its own
 * provider emulator, so the e2e signs in through a real redirect and exchange.
