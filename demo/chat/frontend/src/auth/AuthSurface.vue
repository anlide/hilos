...
import { authGateKey } from './authGateKey'
import {
  abandonRegistration,
  detectIdentifier,
  runAuthMethod,
...
} from './authConvergeSignals'
import { peekOAuthLink } from './oauthLogin'
import { OAUTH_PROVIDERS } from './oauthProviders'
import { connection } from '../bootstrap/connection'
import { pendingAck, pendingRegistration } from '../bootstrap/session'
...
/** Where the full texts the consent screen links to are served. */
const TERMS_PATH = '/terms'

/** Where the privacy policy the consent screen links to is served. */
const PRIVACY_PATH = '/privacy'

// The project's ORDERED enabled methods — the extension point the machine takes
// as data. The identifier method comes first (it owns the shared field), then
// the icon methods in the order their buttons appear; the providers come from
// the project's own declaration, since it is the project that wired the
// credentials. HIL-427 replaces this literal with the settings-owned set.
...
const auth = createAuthFlow({
  ...
  onDetect: (identifier) => detectIdentifier(identifier),
  onSubmit: submitAuthFlow,
  onMethodAction: runAuthMethod,
})

const gate = inject(authGateKey, null)
...
const primaryMethod = computed(() => {
  ...
  return methods.find((method) => method.key === action.key) ?? null
})
...
const deliveredChannel = computed(() => {
  ...
  return codeChannels.find((channel) => channel.key === key)?.label ?? key
})
...
onMounted(() => {
  ...
  stopWatchingConverge = connection.on(
    'projectSignal',
    ...
    >
      ...
      <div class="form-check mb-3">
        ...
        <label class="form-check-label small" for="auth-consent-accept">
          I agree to the
          <a :href="TERMS_PATH" target="_blank" rel="noopener">Terms</a>
          and the
