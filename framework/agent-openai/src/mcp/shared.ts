...
const envSchema = z.object({
  ...
  AGENT_OPENAI_REPORTS_DIR: z.string().trim().default('/data/reports'),
  AGENT_OPENAI_FINGERPRINT_FILE: z.string().trim().default('/data/fingerprints.json'),
...
export function loadMcpRuntimeConfig(): McpRuntimeConfig {
  ...
  return {
    workspaceRoot,
    allowedRoots,
    reportsDir: path.resolve(parsed.AGENT_OPENAI_REPORTS_DIR),
    fingerprintFile: path.resolve(parsed.AGENT_OPENAI_FINGERPRINT_FILE),
...
export async function fingerprintFiles(paths: string[], config: McpRuntimeConfig): Promise<
  Array<{
    relativePath: string;
    absolutePath: string;
    fileMd5: string;
