...
Standalone Node/TypeScript API server for `@openai/agents` with:
...
```bash
docker compose -f framework/docker/agent-openai-local.yml up -d --build
```

The service listens on `9307`.
...
- `AGENT_OPENAI_ALLOWED_ROOTS` default: `/workspace`
- `AGENT_OPENAI_MAX_FILE_SIZE_BYTES` default: `256000`
