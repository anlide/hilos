...
async function main(): Promise<void> {
    ...
    wss.handleUpgrade(request, socket, head, (ws) => {
      ...
      ws.send(
        JSON.stringify({
          type: 'hello',
          payload: {
            message: 'Connected to agent-claude websocket',
          },
        }),
      );
  ...
  server.listen(config.port, config.host, () => {
    console.log(`agent-claude listening on ${config.host}:${config.port}`);
  });
...
main().catch((error) => {
  console.error('agent-claude bootstrap failed', error);
  process.exit(1);
});