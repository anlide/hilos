...
/**
 * Read the project's name from its frontend package-lock.json, falling back
 * to the project directory name when absent or empty.
 *
 * @param {string} frontendDir Path to the demo's frontend directory.
 * @returns {string} The project name.
 */
export function readProjectName(frontendDir) {
  const lock = readJsonOrNull(join(frontendDir, 'package-lock.json'))
  if (lock && typeof lock.name === 'string' && lock.name.trim() !== '') {
    return lock.name.trim()
  }

  return basename(dirname(resolve(frontendDir)))
}
 ...
 */
export function renderInventoryModule(entries, project) {
  const lines = [
