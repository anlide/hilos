...
// (HilosTableLive), not inside this panel. Internal to the Angular view layer on
// purpose: it is not exported from index.ts, for the reason the bar and the footer
// are not. It is MOUNTED for the whole life of a table that declared bulk
// operations and only shows itself while rows are marked. Its confirmation is a
// HilosModal, and a dialog owns the page's scroll lock: it takes the lock when it
// opens, and a closed one releases it the moment it mounts, whosever lock that was.
// A panel that came and went with what the server sends would pass that lock around
// on nobody's behalf, and would take an open dialog off the screen with it. The
// Angular port of the Vue reference (vue/src/HilosTableSelection.vue), under the
// same names and words.
import {
...
@Component({
  ...
  imports: [HilosActionError, HilosModal, LoadingButton],
  template: `
    <div
...
export class HilosTableSelection<R> {
  ...
  protected async submitBulk(): Promise<void> {
    ...
    const handle = this.controller().runBulk(action, runTarget)
