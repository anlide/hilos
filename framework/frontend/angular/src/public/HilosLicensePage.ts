// HilosLicensePage — the tier-2 public /license page: the project's own license
// prose (the projected content) and, under it, the inventory of everything the
// build actually stands on. The list is not typed by hand and not fetched: it is
// the build-time snapshot generated from the project's own two lockfiles
// (framework/frontend/scripts/generate-license-inventory.mjs), handed over in one
// input, so the page renders whole on a machine with no browser and no server
// behind it (build-and-docker.md, the SSG section).
//
// The page holds no collection logic of its own — filtering, the option lists,
// the label and the CSV rendering are @hilos/core, so the three view layers
// cannot drift apart. It carries no `title` input either: the heading moves with
// the frame, and the project's file holds prose alone.
//
// The clipboard is read on the CLICK and never during render: this page is
// executed at build time by a server renderer where `navigator` does not exist at
// all, so a render-time probe would decide the static file's contents by the
// absence of a browser on the build machine, and the button would appear only
// after the SPA mounts. The two export buttons are therefore always rendered and
// never disabled — on an empty result they hand over the header row alone.
// Bootstrap classes only, no CSS of its own (styling-rules.md).
import {
  ChangeDetectionStrategy,
  Component,
  computed,
  effect,
  input,
  signal,
} from '@angular/core'
import {
  type HilosLicenseEntry,
  type HilosLicenseInventory,
  copyToClipboard,
  downloadTextFile,
  filterLicenseEntries,
  licenseCsvFileName,
  licenseFilterOptions,
  licenseLanguageLabel,
  renderLicenseCsv,
} from '@hilos/core'

import { HilosLongText } from '../HilosLongText.js'
import { HilosModal } from '../HilosModal.js'
import { HilosStaticPage } from '../HilosStaticPage.js'

/** The type the export is offered under. */
const DOWNLOAD_MIME_TYPE = 'text/csv;charset=utf-8'

/**
 * The /license page: the project's prose, and under it the build's inventory
 * with a search, two filters, a package's own license text, and both exports.
 */
@Component({
  selector: 'hilos-license-page',
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [HilosLongText, HilosModal, HilosStaticPage],
  template: `
    <hilos-static-page title="License">
      <ng-content />

      <h2 class="h6 text-uppercase text-body-secondary mb-2 mt-4">
        What this build stands on
      </h2>

      <div class="d-flex flex-wrap gap-2 mb-3">
        <div
          class="input-group input-group-sm w-auto flex-grow-1 flex-md-grow-0"
        >
          <span class="input-group-text">
            <i class="bi bi-search" aria-hidden="true"></i>
          </span>
          <!-- The placeholder doubles as the field's accessible name, as it does
          on the table bar: the field carries no visible label, and two different
          strings would name it twice. The two selects are named by their own
          "all" option for the same reason. -->
          <input
            type="search"
            class="form-control"
            placeholder="Search a package or a license"
            aria-label="Search a package or a license"
            data-id="license-search"
            [value]="search()"
            (input)="onSearch($event)"
          />
        </div>
        <select
          class="form-select form-select-sm w-auto"
          aria-label="All licenses"
          data-id="license-filter-license"
          [value]="license()"
          (change)="onLicense($event)"
        >
          <option value="">All licenses</option>
          @for (option of options().licenses; track option) {
            <option [value]="option">{{ option }}</option>
          }
        </select>
        <select
          class="form-select form-select-sm w-auto"
          aria-label="All languages"
          data-id="license-filter-language"
          [value]="language()"
          (change)="onLanguage($event)"
        >
          <option value="">All languages</option>
          @for (option of options().languages; track option) {
            <option [value]="option">{{ languageLabel(option) }}</option>
          }
        </select>
      </div>

      <div data-id="license-inventory">
        @for (
          entry of visible();
          track entry.language + '/' + entry.name + '/' + entry.version
        ) {
          <button
            type="button"
            class="d-flex align-items-center gap-2 py-1 small w-100 text-start border-0 border-bottom bg-transparent"
            data-id="license-row"
            (click)="openEntry.set(entry)"
          >
            <span class="flex-grow-1">
              {{ entry.name }}
              <span class="text-body-secondary">{{ entry.version }}</span>
            </span>
            <span class="badge text-bg-light border">
              {{ languageLabel(entry.language) }}
            </span>
            <span class="badge text-bg-light border">{{ entry.license }}</span>
          </button>
        }
        @if (visible().length === 0) {
          <p class="small text-body-secondary mb-0" data-id="license-empty">
            Nothing in this build matches these filters
          </p>
        }
      </div>

      <div class="d-flex flex-wrap align-items-center gap-2 mt-3">
        <button
          type="button"
          class="btn btn-sm btn-outline-secondary"
          data-id="license-copy"
          (click)="copy()"
        >
          <i class="bi bi-clipboard me-1" aria-hidden="true"></i>
          Copy the list
        </button>
        <button
          type="button"
          class="btn btn-sm btn-outline-secondary"
          data-id="license-download"
          (click)="download()"
        >
          <i class="bi bi-download me-1" aria-hidden="true"></i>
          Download as a file
        </button>
        <!-- Rendered even while empty: a live region has to be in the document
        before its text changes, or the change is never announced. -->
        <span
          class="small text-body-secondary"
          role="status"
          aria-live="polite"
          data-id="license-copy-status"
          >{{ copyStatus() }}</span
        >
      </div>

      <hilos-modal
        [open]="openEntry() !== null"
        (openChange)="onModalOpen($event)"
        [title]="modalTitle()"
        initialFocus="dialog"
        size="wide"
      >
        @if (openEntry(); as entry) {
          @if (entry.licenseText !== null) {
            <hilos-long-text
              kind="output"
              [text]="entry.licenseText"
              dataId="license-text"
            />
          } @else {
            <p class="small mb-0" data-id="license-text-missing">
              This package ships no license file; the type above comes from its
              manifest
            </p>
          }
        }
        <!-- A direct child of the dialog, not a node inside the block above: the
        modal picks its footer up with a content query, and a template declared
        inside a conditional block is not there to be found. -->
        <ng-template #modalActions let-requestClose="requestClose">
          @if (openEntry()?.repository; as repository) {
            <a
              [href]="repository"
              class="btn btn-outline-secondary"
              target="_blank"
              rel="noopener noreferrer"
              data-id="license-repository"
            >
              <i class="bi bi-box-arrow-up-right me-1" aria-hidden="true"></i>
              Repository
            </a>
          }
          <button
            type="button"
            class="btn btn-secondary"
            (click)="requestClose()"
          >
            Close
          </button>
        </ng-template>
      </hilos-modal>
    </hilos-static-page>
  `,
})
export class HilosLicensePage {
  /** The build-time snapshot of everything this build stands on. */
  readonly inventory = input.required<HilosLicenseInventory>()

  protected readonly search = signal('')
  protected readonly license = signal('')
  protected readonly language = signal('')
  protected readonly openEntry = signal<HilosLicenseEntry | null>(null)
  protected readonly copyStatus = signal('')
  protected readonly languageLabel = licenseLanguageLabel

  // Both selects offer what the snapshot actually holds, never a fixed enum: a
  // hard-coded list would silently hide a license nobody expected, which is the
  // one thing this page exists to surface.
  protected readonly options = computed(() =>
    licenseFilterOptions(this.inventory().entries),
  )

  protected readonly visible = computed(() =>
    filterLicenseEntries(this.inventory().entries, {
      search: this.search(),
      license: this.license(),
      language: this.language(),
    }),
  )

  protected readonly modalTitle = computed(() => {
    const entry = this.openEntry()

    return entry === null
      ? ''
      : `${entry.name} ${entry.version} · ${entry.license}`
  })

  constructor() {
    // The status speaks about the list that was handed over, so it goes as soon
    // as the list underneath it changes — the same reason a reopened modal does
    // not keep reporting the copy of its last visit.
    effect(() => {
      this.visible()
      this.copyStatus.set('')
    })
  }

  protected onSearch(event: Event): void {
    this.search.set((event.target as HTMLInputElement).value)
  }

  protected onLicense(event: Event): void {
    this.license.set((event.target as HTMLSelectElement).value)
  }

  protected onLanguage(event: Event): void {
    this.language.set((event.target as HTMLSelectElement).value)
  }

  protected onModalOpen(open: boolean): void {
    if (!open) this.openEntry.set(null)
  }

  protected async copy(): Promise<void> {
    const copied = await copyToClipboard(renderLicenseCsv(this.visible()))
    this.copyStatus.set(
      copied ? 'Copied' : 'The browser did not allow copying — use Download',
    )
  }

  protected download(): void {
    downloadTextFile(
      licenseCsvFileName(this.inventory().project),
      renderLicenseCsv(this.visible()),
      DOWNLOAD_MIME_TYPE,
    )
  }
}
