...
default slot) and, at the end of it, the block offering to support the project.
...
un-interviewed. This page hands it no wire name, no storage and no configuration
to inherit.

The one decision that outlives the stub, and the reason the copy reads the way it
does: the subscription unlocks NOTHING. Every feature is open to everyone without
it, and what it buys is a tick and thanks. A support screen that hints at
unlocking is read as a paywall however it is worded afterwards.

Guest and signed-in see the same block — no per-reader part, no session read —
which is also what keeps /about prerenderable whole: the plate and its button are
in the static HTML, and everything browser-touching happens on the click.
  ...
  <HilosStaticPage title="About">
    ...
    >
      <i
        ...
        aria-hidden="true"
      ></i>
      <h2 class="h6 mb-1">The project runs with no ads and no data selling</h2>
      <p class="small text-body-secondary mb-3">
        If it has been useful to you, you can chip in for its upkeep. In return
        you get nothing but a tick.
      </p>
      <button
        type="button"
