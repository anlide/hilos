...
child pages and puts its own figures underneath them.
...
<template>
  <HilosAdminPage :page="HilosPages.LOGS">
    ...
    <template v-if="clustered">
      ...
      <p class="small text-body-secondary">
        A journal is written on the node the event happened on and stays there.
        The tiles above add every node together; here you can see whose journal
        is growing faster.
      </p>
        ...
        <table class="table table-hover align-middle mb-0">
          <thead>
            <tr>
              ...
              <th scope="col" class="text-end d-none d-lg-table-cell">Live</th>
    ...
    >
      ...
      <template v-if="clustered"> No node could read its log store. </template>
      <template v-else>The log store could not be read.</template>
      Check the log directory setting and the permissions on it. The tiles stay
      empty rather than zero — a zero would be a measurement nobody took.
    </div>
  </HilosAdminPage>
</template>
