...
describe('parseSignal', () => {
  ...
  it('parses a table_window_refused frame as a framework signal', () => {
    const result = parseSignal(
      '{"type":"table_window_refused","data":{"page":"hilos_settings","tableKey":"settings","errorCode":"internal_error"}}',
    )
    expect(result.ok).toBe(true)
    if (result.ok && result.signal.kind === 'tableWindowRefused') {
  ...
  it('rejects a table_window_refused frame without errorCode', () => {
    const result = parseSignal(
      '{"type":"table_window_refused","data":{"page":"hilos_settings","tableKey":"settings"}}',
    )
    expect(result.ok).toBe(false)
    if (!result.ok) {
      expect(result.failure).toMatchObject({
        kind: 'invalid-signal-data',
        type: 'table_window_refused',
      })
    }
  })
