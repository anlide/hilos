 ...
 *
 * One verdict rather than one per door: the CLI base ({@see TestOnlyCommand}) and the
 * command socket both ask the same question, and separate copies are separate chances for a
 * node to be judged differently depending on which door the caller came through. The class
 * lives in the environment namespace and is named after the question it answers, because a
 * verdict named after its first caller invites the second caller to write its own.
 */
