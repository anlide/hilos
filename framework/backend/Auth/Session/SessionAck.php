...
/**
 * The kinds of ack an auth flow can leave on the session it signed in (HIL-422).
 *
 ...
 * for the view, which holds the copy (HIL-423/424/425 draw it). The mark is held by the
 * session row it belongs to (HIL-875), not by the socket that earned it.
 *
 * The registry is a closed set on purpose. The mark travels as one flag on the wire
 * (see {@see SessionActions::holdPendingAck()}), so the frontend maps a value to a screen —
 * and a value it does not know is a screen nobody wrote. Adding a kind therefore means
 * adding its copy on all three views in the same breath.
 *
