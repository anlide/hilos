 ...
 *
 * Opts out of cluster leadership and of a monopolistic worker, the same stub
 * {@see LogRotationAgentDaemon} is, and for a related reason: the attempt counters are
 * node-local. Runtime sync reaches the workers of one node and no further, so an agent
 * pinned to the leader would leave every other node counting nothing. What must be global
 * is not the count but the block, and that is global already - it is a row in the shared
 * database, which every node's agent reads on start.
 */
