 ...
 * policy requires it), verifies the ES256 signature over
...
final class AssertionVerifier
{
     ...
     * @throws WebAuthnVerificationException When any client-data, authenticatorData, signature or counter check fails
     */
    public function verify(
        ...
        $this->verifySignature($publicKeyPem, $authenticatorData, $clientDataJson, $signature);
    ...
    /**
     * Verifies the ES256 signature over authenticatorData concatenated with the client-data hash.
     *
     * @param string $publicKeyPem Stored PEM credential public key
     * @param string $authenticatorData Raw authenticatorData bytes
     * @param string $clientDataJson Raw clientDataJSON bytes
     * @param string $signature Raw ECDSA (DER) signature bytes
     * @throws WebAuthnVerificationException When the key cannot be loaded or the signature does not verify
     */
    private function verifySignature(string $publicKeyPem, string $authenticatorData, string $clientDataJson, string $signature): void
    {
