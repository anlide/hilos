 ...
 * (one {@see GenericOAuthProvider} per provider it holds a client pair for) and
 * hands it to the {@see OAuthService}; the async agent reads providers back out
 * of it by key, and a project reads its enabled auth methods out of {@see keys()}.
 *
 * A provider without its client pair never reaches this registry: the project
 * does not register it, on any node, so it is gone from the service, the agent
 * and the auth methods at once (HIL-924).
 */
final class OAuthProviderRegistry
{
    ...
    /**
     * Registers every provider the project configured, keyed in configuration order.
     *
