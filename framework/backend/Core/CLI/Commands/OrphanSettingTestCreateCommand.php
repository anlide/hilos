...
final class OrphanSettingTestCreateCommand extends TestOnlyCommand
{
    ...
    {
        return 'Test-only: write the example orphan settings row';
    }
...
Writes an orphan settings row for the example catalog key so a test can set up that
state. Restore the baseline with the matching delete command.
HELP;
    }
    ...
    protected function run(array $options, array $args): int
    {
        ...
        Hilos::$db->settings->actions->add(self::ORPHAN_KEY, null, Hilos::$setting->catalog());
        echo "Orphan setting created for key '" . self::ORPHAN_KEY . "'.\n";
