...
/**
 * Header of the log-keys screen (server → client, HIL-385).
 *
 ...
 * ({@see HilosLogKeysTable}) and are deliberately not here — a header that carried rows would have
 * to be re-sent whenever a window was scrolled.
 *
 * {@see $available} has the three answers the rotation screen's has, and they are three different
 * screens. Null: no merged picture has arrived, because the aggregator is unplaced, moving, or
 * simply has not answered yet ({@see ClusterLogIndexMirror}). False: the picture arrived and not
 * one node could read its log store. True: there are streams to list, even if there are none yet.
 *
 ...
 *
 * The rules of rotation and retention are NOT here, though the neighbouring header carries them:
 * this screen shows what the streams cost, and it neither states nor applies a rule.
 */
final class HilosLogsKeysSignalData extends BaseDTO implements SignalDataInterface
{
