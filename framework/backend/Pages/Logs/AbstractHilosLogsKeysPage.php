...
/**
 * Abstract Hilos admin page: the log streams of the installation, one row per key on a node.
 *
 ...
 * it changes. The streams ride the ordinary windowed table ({@see HilosLogKeysTable}) over
...
abstract class AbstractHilosLogsKeysPage extends AbstractHilosPage
{
    ...
    {
        $this->logAgentInfo("hilos_logs_keys onUnsubscribe acceptKey={$acceptKey}");
     ...
     *
     * @return HilosLogsKeysSignalData Header payload
     */
    private static function buildHeader(): HilosLogsKeysSignalData
    {
        $index = ClusterLogIndexMirror::index();

        return new HilosLogsKeysSignalData(
    ...
    /**
     * Re-serves every subscriber's own window of the log-keys table.
     *
    ...
    {
        foreach (array_keys(self::$subscribers) as $acceptKey) {
            $viewport = Hilos::$sr?->getTableViewport($acceptKey, HilosLogKeysTable::TABLE);
            if ($viewport === null) {
                continue;
            }

            Hilos::$browser?->sendTableWindow(HilosPageConstants::HILOS_LOGS_KEYS, $acceptKey, $viewport);
        }
    }

    /**
     * Fingerprint of everything the stream rows are projected from.
     *
     ...
     * a picture that had not changed at all. The daily growth belongs here beside them — it is a
     * column of the table, and it moves without a single stream having been added or rotated.
     *
     * @param HilosLogsKeysSignalData $header Header of this pass, for the availability it carries
     ...
     */
    private static function rowsFingerprint(HilosLogsKeysSignalData $header): string
    {
        $slots = [];
        foreach (ClusterLogIndexMirror::index()?->nodes() ?? [] as $slot) {
            $slots[] = [
                ...
                array_map(
                    static fn(LogKeySummary $summary): array => [
                        $summary->key,
                        $summary->class,
                        $summary->live,
     ...
     * @param HilosLogsKeysSignalData $header Payload
     * @throws InvalidArgumentException When the header signal cannot be named
     */
