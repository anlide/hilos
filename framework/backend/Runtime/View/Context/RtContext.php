...
abstract class RtContext
{
     ...
     *
     * Four of these are mounted unconditionally and before any feature, because none is an
     ...
     * the ordinary cookie rule. The session toast stacks (HIL-768) are there for the same reason
     * as the rotations and are inert in the same way: what a browser is being shown is decided by
     * the session in RT, and a session is not something a project declares.
     * The cluster nodes (HIL-337) are there because clustering is a
    ...
    final public function mountFeatureRuntime(array $definitions): void
    {
        ...
        $this->mountFeatureCollection(
            StateHilosSessionToastStack::RT_COLLECTION,
            StateHilosSessionToastStacks::init(),
        );
        $this->setRepresent(
            StateHilosSessionToastStack::RT_COLLECTION,
            HilosSessionToastStacks::class,
            HilosSessionToastStacksActions::class,
        );
