...
final class HilosLogWorkersTable extends TableDefinition implements ViewportTable
{
    ...
    /**
     * Declares what a worker row is searched by: the key that is the file's name, the node it runs
     * on, and the kind of worker it is.
     *
     * The weight and the batch count are numbers an operator reads and nobody types part of, so
     * they stay out: a short term matching them would match nearly every row.
     *
    ...
    protected function searchableFields(): array
    {
        return [
            HilosLogWorkersTableRow::key => HilosLogWorkersTableRow::key,
            HilosLogWorkersTableRow::node => HilosLogWorkersTableRow::node,
            HilosLogWorkersTableRow::type => HilosLogWorkersTableRow::type,
        ];
    }
    ...
    /**
     * Applies the node filter and the type filter this screen answers to.
     *
