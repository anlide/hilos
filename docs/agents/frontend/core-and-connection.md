...
| the verification window opening | the ordinary application, for the browser that started the operation, for every member of the circle photographed at the freeze, and for any browser holding a code: a frame of its own reaches that session, so every tab of it leaves the stub at once and none of them needs an F5. The page behind the stub is answered too — by the server for a page it already held, and by the tab itself for a page subscribe the freeze refused: `PageSubscription` sends it when the mode stops holding the connection; the notification bell joins its group again at the same moment, refused or not, because nothing on the server answers a group a second time (HIL-1079) |
