...
| **A column of a stale source is not sorted at all.** No mechanism of its own: this is the rule of [Per-source staleness](table-subscription.md#per-source-staleness). The view refuses it by drawing **no sort control** on that header — not a disabled one: a disabled button drops out of the focus order, so the reason hung on it would never reach the one reader who needs it most. In its place stand the column label, the snowflake, and the three parts of the refusal as hidden text beside it; the arrow of a standing order stays, no longer a button. | An order over stale values is indistinguishable from a wrong one. | "This column's source is lagging right now — an order over it would be a lie." |
