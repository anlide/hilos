...
> stays current, a copy in a doc does not. One case is named here all the same,
> because it decides what an agent may promise today: until a table can declare
> a composite order, it declares single-column orders only — the map in
> `TableDefinition::sortableFields()` names columns, and `TableSortWhitelist`
> holds every query path to that map (not in the code yet — HIL-789).
...
| **The last component of every order is the primary key.** Today the ORM settles it (HIL-786): `Objects::queryPage()` appends the primary-key columns in the direction of the sorted column. | Without it the order is not total: on a column with repeats two adjacent pages show one row twice and another not at all, and the server cannot say where an arriving row falls relative to the window. | "I am adding the primary key at the end — without it the window drifts." |
