...
| [phpdoc.md](phpdoc.md) | PHP | writing or changing PHPDoc, overriding inherited methods, adding `@see` links, explaining in a comment why a value can be trusted, or describing in a comment what the screen shows |
...
| [catalog-backed-accessors.md](catalog-backed-accessors.md) | PHP | reading a value out of `Hilos::$env` or `Hilos::$setting`, or building a third accessor of that kind — the index is the one way in, a typed method beside it is not added, and the catalog is asked once per instance |
...
| [scaffold-markers.md](scaffold-markers.md) | both | leaving code/config wired but intentionally unused — marking a scaffold or a deliberate keep so a dead-code sweep does not cull it — or marking a test stub as a sunset the stand's emulated services will replace |
