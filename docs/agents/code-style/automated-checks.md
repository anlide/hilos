...
| `THROWS-PROPAGATION` | An exception a callee documents is named by the caller's own `@throws` too, unless an enclosing `catch` swallows it; and an implementation does not document an exception the declaration it overrides is silent about. A `throw new X` is judged as its own callee. Only calls whose target is known without inferring a type; an index on such a receiver counts as the call it is, reaching one of the four `ArrayAccess` methods; a magic property's class may be named by a constant on its receiver; a private helper is walked through rather than trusted. | [phpdoc.md](phpdoc.md) |
