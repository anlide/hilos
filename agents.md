...
| [architecture/verification-codes.md](docs/agents/architecture/verification-codes.md) | issuing, checking or spending a one-time code or link: the atomic single-use spend and its outcome, the attempt ceiling, and the send cap that is deliberately not atomic; also where a code issued on a stand is read: mail, SMS and Telegram all end in the stand's Mailpit, and no file on disk carries it |
