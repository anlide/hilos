...
| [architecture/uploads.md](docs/agents/architecture/uploads.md) | receiving a file over frame_binary: declaring it, signed chunks, checks, the upload's state on the wire, cleanup |
| [architecture/files-registry.md](docs/agents/architecture/files-registry.md) | storing, binding or sweeping a published file: the one owner of `hilos_file`, the project's `markBound()` after its own link, the janitor's row-then-file order and the foreign-key refusal, the reserved files directory |
