...
| [architecture/files-registry.md](docs/agents/architecture/files-registry.md) | publishing a received upload, storing, binding or sweeping a published file: the one owner of `hilos_file`, the storage seam, the project's `markBound()` after its own link, the janitor's row-then-file order and the foreign-key refusal, the reserved files directory |
