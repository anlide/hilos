...
5. Use typed parameters, DTOs, value objects, or typed collections for internal
   backend API; keep unstructured arrays at boundaries.
6. Avoid one-use locals unless they improve clarity under the local variable rule.
7. Keep comments concise and in English.
8. For `Page::onAction()`, do not add a local `try/catch` around the routing
   `switch`; the framework catches action exceptions and calls
   `onActionException()`.
9. Override `onActionException()` only when the page has a specific fail/error
   contract; otherwise let the default framework `action_error` signal notify
   the initiator.
10. Use `AgentUnknownActionException` in `onAction()` default branches.
11. In PHPDoc, import exception classes with `use` and reference short names;
   do not write leading-backslash fully qualified exceptions such as
   `@throws \OutOfBoundsException`.
12. Use `ValidationException` and its children for user/business validation;
