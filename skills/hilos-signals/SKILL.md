...
3. Define or update payload DTOs for new wire contracts.
4. Keep serialization roundtrips explicit with `toArray()` and `fromArray()` where applicable.
5. If the signal crosses worker-to-daemon IPC, add backend roundtrip coverage.
6. If the signal reaches frontend code, add or update the TypeScript parser tests.
